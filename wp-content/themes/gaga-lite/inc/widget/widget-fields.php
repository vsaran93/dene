 ...
 * @package gaga lite
 */
function gaga_lite_widgets_show_widget_field($instance = '', $widget_field = '', $athm_field_value = '') {
    // Store Posts in array
    $gaga_lite_postlist[0] = array(
        'value' => 0,
        'label' => '--choose--'
    );
    $arg = array('posts_per_page' => -1);
    $gaga_lite_posts = get_posts($arg);
    foreach ($gaga_lite_posts as $gaga_lite_post) :
        $gaga_lite_postlist[$gaga_lite_post->ID] = array(
            'value' => $gaga_lite_post->ID,
            'label' => $gaga_lite_post->post_title
        );
    endforeach;

    extract($widget_field);

    switch ($gaga_lite_widgets_field_type) {

        // Standard text field
        case 'text' :
            ?>
            <p>
                <label for="<?php echo $instance->get_field_id($gaga_lite_widgets_name); ?>"><?php echo wp_kses_post($gaga_lite_widgets_title); ?>:</label>
                <input class="widefat" id="<?php echo $instance->get_field_id($gaga_lite_widgets_name); ?>" name="<?php echo $instance->get_field_name($gaga_lite_widgets_name); ?>" type="text" value="<?php echo $athm_field_value; ?>" />

                <?php if (isset($gaga_lite_widgets_description)) { ?>
                    <br />
                    <small><?php echo wp_kses_post($gaga_lite_widgets_description); ?></small>
                <?php } ?>
            </p>
            <?php
            break;
        case 'color_peacker' :
            ...
            <p>
                <label for="<?php echo $instance->get_field_id($gaga_lite_widgets_name); ?>"><?php echo wp_kses_post($gaga_lite_widgets_title); ?>:</label>
                <input class="widefat my-color-picker" id="<?php echo $instance->get_field_id($gaga_lite_widgets_name); ?>" name="<?php echo $instance->get_field_name($gaga_lite_widgets_name); ?>" type="text" value="<?php echo $athm_field_value; ?>" />
        ...
        case 'textarea' :
            ...
            <p>
                <label for="<?php echo $instance->get_field_id($gaga_lite_widgets_name); ?>"><?php echo wp_kses_post($gaga_lite_widgets_title); ?>:</label>
                <textarea class="widefat" rows="<?php echo $gaga_lite_widgets_row; ?>" id="<?php echo $instance->get_field_id($gaga_lite_widgets_name); ?>" name="<?php echo $instance->get_field_name($gaga_lite_widgets_name); ?>"><?php echo wp_kses_post($athm_field_value); ?></textarea>
            </p>
            <?php
            break;
        ...
        case 'checkbox' :
            ?>
            <p>
                <input id="<?php echo $instance->get_field_id($gaga_lite_widgets_name); ?>" name="<?php echo $instance->get_field_name($gaga_lite_widgets_name); ?>" type="checkbox" value="1" <?php checked('1', $athm_field_value); ?>/>
                <label for="<?php echo $instance->get_field_id($gaga_lite_widgets_name); ?>"><?php echo wp_kses_post($gaga_lite_widgets_title); ?></label>
        ...
        case 'radio' :
            ...
            <p>
                <?php
                echo wp_kses_post($gaga_lite_widgets_title);
                echo '<br />';
                foreach ($gaga_lite_widgets_field_options as $athm_option_name => $athm_option_title) {
                    ?>
                    <input id="<?php echo $instance->get_field_id($athm_option_name); ?>" name="<?php echo $instance->get_field_name($gaga_lite_widgets_name); ?>" type="radio" value="<?php echo $athm_option_name; ?>" <?php checked($athm_option_name, $athm_field_value); ?> />
                    <label for="<?php echo $instance->get_field_id($athm_option_name); ?>"><?php echo wp_kses_post($athm_option_title); ?></label>
                    <br />
                <?php } ?>
        ...
        case 'select' :
            ...
            <p>
                <label for="<?php echo $instance->get_field_id($gaga_lite_widgets_name); ?>"><?php echo wp_kses_post($gaga_lite_widgets_title); ?>:</label>
                <select name="<?php echo $instance->get_field_name($gaga_lite_widgets_name); ?>" id="<?php echo $instance->get_field_id($gaga_lite_widgets_name); ?>" class="widefat">
                    <?php foreach ($gaga_lite_widgets_field_options as $athm_option_name => $athm_option_title) { ?>
                        <option value="<?php echo $athm_option_name; ?>" id="<?php echo $instance->get_field_id($athm_option_name); ?>" <?php selected($athm_option_name, $athm_field_value); ?>><?php echo wp_kses_post($athm_option_title); ?></option>
                    <?php } ?>
                </select>
        ...
        case 'number' :
            ?>
            <p>
                <label for="<?php echo $instance->get_field_id($gaga_lite_widgets_name); ?>"><?php echo wp_kses_post($gaga_lite_widgets_title); ?>:</label><br />
                <input name="<?php echo $instance->get_field_name($gaga_lite_widgets_name); ?>" type="number" step="1" min="1" id="<?php echo $instance->get_field_id($gaga_lite_widgets_name); ?>" value="<?php echo wp_kses_post($athm_field_value); ?>" class="small-text" />
        ...
        case 'selectpost' :
            ...
            <p>
                <label for="<?php echo $instance->get_field_id($gaga_lite_widgets_name); ?>"><?php echo wp_kses_post($gaga_lite_widgets_title); ?>:</label>
                <select name="<?php echo $instance->get_field_name($gaga_lite_widgets_name); ?>" id="<?php echo $instance->get_field_id($gaga_lite_widgets_name); ?>" class="widefat">
                    <?php foreach ($gaga_lite_postlist as $gaga_lite_single_post) { ?>
                        <option value="<?php echo $gaga_lite_single_post['value']; ?>" id="<?php echo $instance->get_field_id($gaga_lite_single_post['label']); ?>" <?php selected($gaga_lite_single_post['value'], $athm_field_value); ?>><?php echo wp_kses_post($gaga_lite_single_post['label']); ?></option>
                    <?php } ?>
                </select>
        ...
        case 'upload' :
            ...
            $output = '';
            $id = $instance->get_field_id($gaga_lite_widgets_name);
            $class = '';
            $int = '';
            $value = $athm_field_value;
            $name = $instance->get_field_name($gaga_lite_widgets_name);
            ...
            $output .= '<label for="' . $instance->get_field_id($gaga_lite_widgets_name) . '">' . $gaga_lite_widgets_title . '</label><br/>';
            $output .= '<input id="' . $id . '" class="upload' . $class . '" type="text" name="' . $name . '" value="' . $value . '" placeholder="' . __('No file chosen', 'gaga-lite') . '" />' . "\n";
            if (function_exists('wp_enqueue_media')) {
                //if (( $value == '')) {
                    $output .= '<input id="upload-' . $id . '" class="upload-button button" type="button" value="' . __('Upload', 'gaga-lite') . '" />' . "\n";
                //} else {
                    //$output .= '<input id="remove-' . $id . '" class="remove-file button" type="button" value="' . __('Remove', 'gaga-lite') . '" />' . "\n";
                //}
            } else {
                $output .= '<p><i>' . __('Upgrade your version of WordPress for full media support.', 'gaga-lite') . '</i></p>';
            }

            $output .= '<div class="screenshot team-thumb" id="' . $id . '-image">' . "\n";

            if ($value != '') {
                $remove = '<a class="remove-image">'. __('Remove', 'gaga-lite').'</a>';
                $attachment_id = attachment_url_to_postid($value);
                $image_array = wp_get_attachment_image_src($attachment_id, 'medium');
                $image = preg_match('/(^.*\.jpg|jpeg|png|gif|ico*)/i', $value);
                if ($image) {
                    $output .= '<img width="276px" src="' . $image_array[0] . '" alt="" />' . $remove;
                } else {
                    $parts = explode("/", $value);
        ...
        case 'icon' :
            ...
            <p>
                ...
                <input class="hidden-icon-input" name="<?php echo $instance->get_field_name($gaga_lite_widgets_name); ?>" type="hidden" id="<?php echo $instance->get_field_id($gaga_lite_widgets_name); ?>" value="<?php echo $athm_field_value; ?>" />
...
function gaga_lite_widgets_updated_field_value($widget_field, $new_field_value) {

    extract($widget_field);

    // Allow only integers in number fields
    if ($gaga_lite_widgets_field_type == 'number') {
        return absint($new_field_value);

        // Allow some tags in textareas
    } elseif ($gaga_lite_widgets_field_type == 'textarea') {
        // Check if field array specifed allowed tags
        if (!isset($gaga_lite_widgets_allowed_tags)) {
            // If not, fallback to default tags
            $gaga_lite_widgets_allowed_tags = '<p><strong><em><a>';
        }
        return strip_tags($new_field_value, $gaga_lite_widgets_allowed_tags);

        // No allowed tags for all other fields
    } elseif ($gaga_lite_widgets_field_type == 'url') {
